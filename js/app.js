  ...
  if (importTripsFile) importTripsFile.addEventListener('change', async (e) => {
    ...
    try {
      ...
      const applyMerge = (replace) => {
        ...
        if (user?.id) api.saveTrips(user.id, merged).catch(() => showToast('雲端同步失敗，資料已暫存本機', 'warn'));
...
function bindExpenseFormEvents(trip, existingExp = null) {
  ...
  if (saveBtn) saveBtn.addEventListener('click', async () => {
    ...
    const currency = (q(SEL.efCurrency)?.value?.trim() || trip.base_currency || 'TWD').toUpperCase();
